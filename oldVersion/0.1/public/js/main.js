		...
		for (x = 0; x < Object.keys(scenario.base).length; x++) {
			for (y = 0; y < Object.keys(scenario.base[x]).length; y++) {
				if (scenario.base[x][y] != 0)
					PLATS.push(new object(x * 16, y * 16, scenario.base[x][y] - 1))
			}
		}
	} catch (error) {
		console.log(error)
...
function animation() {
	...
	PLATS.forEach((PLAT) => {
		if (
			...
			const overlapX =
				Math.min(MEGAMAN.posX + MEGAMAN.width, PLAT.posX + PLAT.width) -
				Math.max(MEGAMAN.posX, PLAT.posX)
			const overlapY =
				Math.min(MEGAMAN.posY + MEGAMAN.height, PLAT.posY + PLAT.height) -
				Math.max(MEGAMAN.posY, PLAT.posY)
